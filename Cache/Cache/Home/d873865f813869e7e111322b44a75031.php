...
<div class="menu">
	<ul>
		<li class="menuout"><a href="<?php echo U('Index/best');?>" id="highlight">热门</a></li>
...
<div class="content-block">
<div class="col1">
<div class="block untagged noline">
<div class="author" style="height:30px;line-height:30px;margin:0;">
	<div style="float:left;line-height:30px;margin-left:10px;">
		<a href="<?php echo U('Home/Index/news?id='.$news['id']);?>" id="title-<?php echo ($news["id"]); ?>"><?php echo (msubstr($news["title"],0,25,'utf-8',false)); ?></a>
	</div>
	<div style="float:right;">
		<a href="<?php echo U('User/Index/index?uid='.$news['uid']);?>"><?php echo ($news["uname"]); ?></a>&nbsp;&nbsp;
		<img src="<?php echo ($YICMS_PUBLIC); ?>/face/f_<?php echo ($news["face"]); ?>" alt="<?php echo ($news["uname"]); ?>">
	</div>
	<div style="float:right;line-height:30px;"><?php echo (switchtime($news["time"])); ?>&nbsp;&nbsp;&nbsp;&nbsp;</div>
</div>
<div style="clear:both;"></div>
<div style="height:10px;line-height:10px;background:url('<?php echo ($HOME_PUBLIC); ?>/images/line.gif') repeat-x"></div>
<div class="content" title="<?php echo (switchtime($news["time"])); ?>" id="content-<?php echo ($news["id"]); ?>">
	<?php echo ($news["content"]); ?>
</div>
<?php if(!empty($news['pic'])): ?><div class="thumb">
		<img src="<?php echo ($YICMS_PUBLIC); ?>/news/m_<?php echo ($news["pic"]); ?>"/>
	</div><?php endif; ?>
<?php if(!empty($news['video'])): ?><embed src="<?php echo ($news["video"]); ?>" type="application/x-shockwave-flash" width="550" height="400" quality="high" allowfullscreen="true"/><?php endif; ?>
<?php if(!empty($news['tag'])): ?><div class="tags">
		<span class="bti"></span>
		<?php if(is_array($news["tag"])): $i = 0; $__LIST__ = $news["tag"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Index/tag?name='.$vo);?>"><?php echo ($vo); ?></a>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
	</div><?php endif; ?>
<div class="bar clearfix" id="qiushi_counts_5156003">
	<div class="up">
		<a href="javascript:vote3(<?php echo ($news["id"]); ?>,1)" id="yes-<?php echo ($news["id"]); ?>"><?php echo ($news["yes"]); ?></a>
	</div>
	<div class="down" style="margin:0 302px 0 10px;">
		<a href="javascript:vote3(<?php echo ($news["id"]); ?>,-1)" id="no-<?php echo ($news["id"]); ?>"><?php echo ($news["no"]); ?></a>
	</div>
	<div class="detail" style="width:80px;position: relative;">
		...
		<div style="position:absolute;left:0px;border:1px solid #DFD5CB;display:none;width:80px;margin:29px 0 0;background: white;text-indent: 0;z-index:999;" >
			<ul>
				<li style="height:30px;"><a href="javascript:sharetosina(<?php echo ($news["id"]); ?>)" style="padding: 8px 12px;background: none;border-radius: 0;float: none;height: auto;line-height: 100%;text-indent: 0;width: auto;border: 0 none;z-index:999;border-bottom: 1px solid #ECE5D8;box-shadow: 0 0 0;cursor: pointer;">新浪微博</a></li>
				<li style="height:30px;"><a href="javascript:sharetoqzone(<?php echo ($news["id"]); ?>)" style="padding: 8px 12px;background: none;border-radius: 0;float: none;height: auto;line-height: 100%;text-indent: 0;width: auto;border: 0 none;z-index:999;border-bottom: 1px solid #ECE5D8;box-shadow: 0 0 0;cursor: pointer;">&nbsp;QQ空间</a></li>
				<li style="height:30px;"><a href="javascript:sharetotqq(<?php echo ($news["id"]); ?>)" style="padding: 8px 12px;background: none;border-radius: 0;float: none;height: auto;line-height: 100%;text-indent: 0;width: auto;border: 0 none;z-index:999;border-bottom: 1px solid #ECE5D8;box-shadow: 0 0 0;cursor: pointer;">腾讯微博</a></li>
			</ul>
		</div>
	</div>
</div>
<div class="comments">
<div>
	<?php if(is_array($reply)): $i = 0; $__LIST__ = $reply;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div id="comment-11045137" class="comment-block floor-71">
			<div class="clearfix">
				<div class="avatar">
				...
				<div class="reply">
					<span class="userlogin">
						<a href="<?php echo U('User/Index/index?id='.$vo['uid']);?>"><?php echo ($vo["uname"]); ?></a>
					</span>
					<span class='body'><?php echo ($vo["arcontent"]); ?></span>
				</div>
				<div class="report">
					<!--a href="javascript:showReport(<?php echo ($vo["id"]); ?>)" class="link">举报</a-->
					<span title="<?php echo (date($vo["artime"],'Y-m-d H:i:s')); ?>"><?php echo ($i); ?></span>
				</div>
			</div>
		</div><?php endforeach; endif; else: echo "" ;endif; ?>
</div>
	<div class="input-block">
		<?php if(empty($_SESSION['uid'])): ?><div class="logout login-block"><a href="javascript:showLoginForm()">登录</a>后才能发表评论</div>
		<?php else: ?>	
			<form method="post" action="<?php echo U('Index/checkReply');?>">
				<input type="hidden" name="pid" value="<?php echo ($_GET["id"]); ?>">
				<textarea class="comment_input original" name="arcontent">请不要发表与本内容无关的评论，您有了账号就是有身份的人了，我们可认识您。</textarea>
				<div class="row">
				<button type="submit" id='reply_submit'>发表</button>
					<label for='comment_anonymous'><input id='comment_anonymous' type="checkbox" value="1" name="anonymous"> 匿名评论</label>
				</div>
			</form><?php endif; ?>	
	</div>
</div>
