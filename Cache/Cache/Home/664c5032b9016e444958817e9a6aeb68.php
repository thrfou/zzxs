...
<div class="menu">
	<ul>
		...
		<?php $_result=D('Nav')->nlist('2','2','-1','15','30','1','0'); if($_result): $i=0;foreach($_result as $key=>$nlist):++$i;?><li class="menuout"><a href="<?php echo ($nlist["url"]); ?>" <?php if($_GET['id'] == 2): ?>id="highlight"<?php endif; ?>><?php echo ($nlist["name"]); ?></a></li><?php endforeach; endif;?>
		<?php $_result=D('Nav')->nlist('2','3','-1','15','30','1','0'); if($_result): $i=0;foreach($_result as $key=>$nlist):++$i;?><li class="menuout"><a href="<?php echo ($nlist["url"]); ?>" <?php if($_GET['id'] == 3): ?>id="highlight"<?php endif; ?>><?php echo ($nlist["name"]); ?></a></li><?php endforeach; endif;?>	
		<?php $_result=D('Nav')->nlist('2','4','-1','15','30','1','0'); if($_result): $i=0;foreach($_result as $key=>$nlist):++$i;?><li class="menuout"><a href="<?php echo ($nlist["url"]); ?>" <?php if($_GET['id'] == 4): ?>id="highlight"<?php endif; ?>><?php echo ($nlist["name"]); ?></a></li><?php endforeach; endif;?>
...
<div class="content-block">
<div class="col1">
<?php $_result=D('Nav')->nlist('3','4','-1','25','1000000','1','-1'); if($_result): $i=0;$page=$_result["page"];foreach($_result["news"] as $key=>$nlist):++$i;?><div class="block untagged" id="qiushi-<?php echo ($nlist["id"]); ?>">
<div class="author" style="height:30px;line-height:30px;margin:0;">
	<div style="float:left;line-height:30px;margin-left:10px;">
		<a href="<?php echo U('Home/Index/news?id='.$nlist['id']);?>" id="title-<?php echo ($nlist["id"]); ?>" target="_blank"><?php echo ($nlist["title"]); ?></a>
	</div>
	<div style="float:right;">
		<a href="<?php echo U('User/Index/index?uid='.$nlist['uid']);?>" target="_blank"><?php echo ($nlist["uname"]); ?></a>&nbsp;&nbsp;
		<img src="<?php echo ($YICMS_PUBLIC); ?>/face/f_<?php echo ($nlist["face"]); ?>" alt="<?php echo ($nlist["uname"]); ?>">
	</div>
	<div style="float:right;line-height:30px;"><?php echo (switchtime($nlist["time"])); ?>&nbsp;&nbsp;&nbsp;&nbsp;</div>
</div>
<div style="clear:both;"></div>
<div style="height:10px;line-height:10px;background:url('<?php echo ($HOME_PUBLIC); ?>/images/line.gif') repeat-x"></div>
<div class="content" title="<?php echo (switchtime($nlist["time"])); ?>" id="content-<?php echo ($nlist["id"]); ?>">
	<?php echo ($nlist["content"]); ?>	
</div>
<?php if(!empty($nlist['pic'])): ?><div class="thumb">
		<img src="<?php echo ($YICMS_PUBLIC); ?>/news/m_<?php echo ($nlist["pic"]); ?>"/>
	</div><?php endif; ?>
<?php if(!empty($nlist['video'])): ?><embed src="<?php echo ($nlist["video"]); ?>" type="application/x-shockwave-flash" width="550" height="400" quality="high" allowfullscreen="true"/><?php endif; ?>
<?php if(!empty($nlist['tag'])): ?><div class="tags">
	<span class="bti"></span>
	<?php $tag = explode(' ',$nlist['tag']); $max = count($tag); for($i=0;$i<$max;$i++){ echo '<a href="'.U('Index/tag?name='.$tag[$i]).'" target="_blank">'.$tag[$i].'</a>&nbsp;'; } ?>
</div><?php endif; ?>

<div class="bar clearfix">
	<div class="up">
		<a href="javascript:vote3(<?php echo ($nlist["id"]); ?>,1)" id="yes-<?php echo ($nlist["id"]); ?>"><?php echo ($nlist["yes"]); ?></a>
	</div>
	<div class="down" style="margin:0 232px 0 10px;">
		<a href="javascript:vote3(<?php echo ($nlist["id"]); ?>,-1)" id="no-<?php echo ($nlist["id"]); ?>"><?php echo ($nlist["no"]); ?></a>
	</div>
	<div class="comment">
		<a href="javascript:showReply(<?php echo ($nlist["id"]); ?>)" id="showReply-<?php echo ($nlist["id"]); ?>">
			<?php if(empty($nlist['reply'])): ?>评论
			<?php else: ?>
				<?php echo ($nlist["reply"]); endif; ?>
		</a>
	</div>
	<div class="detail" style="width:80px;position: relative;">
		...
		<div style="position:absolute;left:0px;border:1px solid #DFD5CB;display:none;width:80px;margin:29px 0 0;background: white;text-indent: 0;z-index:999;" >
			<ul>
				<li style="height:30px;"><a href="javascript:sharetosina(<?php echo ($nlist["id"]); ?>)" style="padding: 8px 12px;background: none;border-radius: 0;float: none;height: auto;line-height: 100%;text-indent: 0;width: auto;border: 0 none;z-index:999;border-bottom: 1px solid #ECE5D8;box-shadow: 0 0 0;cursor: pointer;">新浪微博</a></li>
				<li style="height:30px;"><a href="javascript:sharetoqzone(<?php echo ($nlist["id"]); ?>)" style="padding: 8px 12px;background: none;border-radius: 0;float: none;height: auto;line-height: 100%;text-indent: 0;width: auto;border: 0 none;z-index:999;border-bottom: 1px solid #ECE5D8;box-shadow: 0 0 0;cursor: pointer;">&nbsp;QQ空间</a></li>
				<li style="height:30px;"><a href="javascript:sharetotqq(<?php echo ($nlist["id"]); ?>)" style="padding: 8px 12px;background: none;border-radius: 0;float: none;height: auto;line-height: 100%;text-indent: 0;width: auto;border: 0 none;z-index:999;border-bottom: 1px solid #ECE5D8;box-shadow: 0 0 0;cursor: pointer;">腾讯微博</a></li>
			</ul>
		</div>
	</div>
</div>
</div>
<div class="shadow"></div><?php endforeach; endif;?>
