...
<div id="right">
	<?php if(empty($articles)): ?><div><h2><?php echo ($profile["uname"]); ?>.没有发布过文章</h2></div>
	<?php else: ?>
		<div><h2><?php echo ($profile["uname"]); ?>.发布的文章</h2></div>
		<ul>
			<?php if(is_array($articles)): $i = 0; $__LIST__ = $articles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
					<span class="user"><?php echo ($vo["author"]); ?></span>
					<span class="time"><?php echo (switchtime($vo["time"])); ?></span><br>
					@<a href="<?php echo U('Home/Index/news?id='.$vo['id']);?>" target="_blank"><?php echo ($vo["title"]); ?></a>
					<!--
					<?php if($_SESSION['uid'] == $vo['uid']): ?>&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo U('Home/Index/edit?id='.$vo['id']);?>" style="color:red;">【点击修改】</a><?php endif; ?>
					-->
					<br/>
