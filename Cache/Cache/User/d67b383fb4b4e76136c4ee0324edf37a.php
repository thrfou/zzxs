...
<div id="right">
	<?php if(empty($comments)): ?><div><h2><?php echo ($profile["uname"]); ?>.没有发布过评论</h2></div>
	<?php else: ?>
		<div><h2><?php echo ($profile["uname"]); ?>.发布的评论</h2></div>
		<ul>
			<?php if(is_array($comments)): $i = 0; $__LIST__ = $comments;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
					<span class="user"><?php echo ($vo["arname"]); ?></span> 
					<span class="time"><?php echo (switchtime($vo["artime"])); ?></span><br>
					@<a href="<?php echo U('Home/Index/news?id='.$vo['pid']);?>"><?php echo (msubstr($vo["arcontent"],0,40,'utf-8',false)); ?></a>
